...
static struct clv_box empty_box = {
	.p1 = {
		.x = 0,
		.y = 0,
	},
...
static struct clv_box *empty_box_ptr = &empty_box;

static struct clv_region_data empty_data = {
	.size = 0,
	.count_boxes = 0,
};

static struct clv_region_data *empty_data_ptr = &empty_data;

static struct clv_region_data broken_data = {
	.size = 0,
	.count_boxes = 0,
};

static struct clv_region_data *broken_data_ptr = &broken_data;
...
#define REGION_BOXES(reg) \
	((reg)->data ? (struct clv_box *)((reg)->data + 1) \
	    : &(reg)->extents)
#define REGION_BOX_PTR(reg) ((struct clv_box *)((reg)->data + 1))
...
#define DOWNSIZE(reg, count_boxes) do { \
	if (((count_boxes) < ((reg)->data->size >> 1)) \
			&& ((reg)->data->size > 50)) { \
		struct clv_region_data * new_data; \
		u32 data_size = REGION_SZOF(count_boxes); \
		if (!data_size) { \
			new_data = NULL; \
		} else { \
			new_data = (struct clv_region_data *) \
...
void clv_region_init_rect(struct clv_region *region,
			     s32 x, s32 y,
			     u32 w, u32 h)
{
	...
	if (!GOOD_BOX(&region->extents)) {
		clv_err("Invalid rectangle, %d,%d %ux%u", x, y, w, h);
		clv_region_init(region);
		return;
	}
...
{
	u32 size = n * sizeof(struct clv_box);

	if (n > UINT32_MAX / sizeof(struct clv_box))
		return 0;

	if (sizeof(struct clv_region_data) > UINT32_MAX - size)
		return 0;

	return size + sizeof(struct clv_region_data);
}

static struct clv_region_data * alloc_data(u32 n)
{
...
static s32 set_break(struct clv_region *region)
{
	FREE_DATA(region);
	region->extents = *empty_box_ptr;
...
static s32 box_alloc(struct clv_region *region, s32 n)
{
...
static inline s32 coalesce(struct clv_region *region,
			   s32 prev_start, s32 cur_start)
{
	struct clv_box *prev_box, *cur_box;
	s32 count_boxes, y2;
...
#define APPEND_REGIONS(new_reg, r, r_end) do { \
	...
	if ((new_boxes = r_end - r)) { \
		...
		memmove((char *)REGION_BOX_TOP(new_reg), (char *)r, \
			new_boxes * sizeof(struct clv_box)); \
		new_reg->data->count_boxes += new_boxes; \
	} \
} while (0)

static inline s32 region_append_non_o(struct clv_region *region,
				      struct clv_box *r,
				      struct clv_box *r_end,
				      s32 y1, s32 y2)
{
	struct clv_box *next_box;
	s32 new_boxes;
...
typedef s32 (*overlap_proc_ptr)(struct clv_region *region,
				struct clv_box *r1,
				struct clv_box *r1_end,
				struct clv_box *r2,
				struct clv_box *r2_end,
				s32 y1,
				s32 y2);

static s32 region_op(struct clv_region *new_reg,
		     struct clv_region *reg1,
		     struct clv_region *reg2,
		     overlap_proc_ptr overlap_func,
		     s32 append_non1,
		     s32 append_non2)
{
	struct clv_box *r1;       /* Pointer into first region     */
	struct clv_box *r2;       /* Pointer into 2d region	     */
	struct clv_box *r1_end;   /* End of 1st region	     */
	struct clv_box *r2_end;   /* End of 2d region		     */
	s32 ybot;                       /* Bottom of intersection	     */
	s32 ytop;                       /* Top of intersection	     */
	struct clv_region_data *old_data; /* Old data for new_reg     */
	s32 prev_band;                  /* Index of start of
				     * previous band in new_reg       */
	s32 cur_band;                   /* Index of start of current
				     * band in new_reg		     */
	struct clv_box * r1_band_end; /* End of current band in r1     */
	struct clv_box * r2_band_end; /* End of current band in r2     */
	...
	old_data = (struct clv_region_data *)NULL;
	...
	if (!(count_boxes = new_reg->data->count_boxes)) {
		...
		FREE_DATA(new_reg);
		new_reg->data = (struct clv_region_data *)NULL;
	} else {
...
static void quick_sort_boxes(struct clv_box boxes[], s32 count_boxes)
{
	s32 y1, x1, i, j;
	struct clv_box *r;
...
static s32 region_subtract_o(struct clv_region *region,
			     struct clv_box *r1,
			     struct clv_box *r1_end,
			     struct clv_box *r2,
			     struct clv_box *r2_end,
...
static s32 region_intersect_o(struct clv_region *region,
			      struct clv_box *r1,
			      struct clv_box *r1_end,
			      struct clv_box *r2,
			      struct clv_box *r2_end,
...
static s32 region_union_o(struct clv_region *region,
			  struct clv_box *   r1,
			  struct clv_box *   r1_end,
			  struct clv_box *   r2,
			  struct clv_box *   r2_end,
...
static s32 validate(struct clv_region *badreg)
{
	/* Descriptor for regions under construction  in Step 2. */
	typedef struct {
		struct clv_region reg;
		s32 prev_band;
		s32 cur_band;
	} region_info_t;
	...
	struct clv_region *reg;             /* ri[j].reg		    */
	struct clv_box *box;                /* Current box in rects	    */
	struct clv_box *ri_box;             /* Last box in ri[j].reg	    */
	struct clv_region *hreg;            /* ri[j_half].reg	    */
	s32 ret = 0;
	...
	if (badreg->extents.p1.x < badreg->extents.p2.x) {
		if ((count_boxes) == 1) {
			FREE_DATA(badreg);
			badreg->data = (struct clv_region_data *)NULL;
		} else {
	...
	for (i = count_boxes; --i > 0;) {
		...
		rit->cur_band = 0;
		rit->reg.extents = *box;
		rit->reg.data = (struct clv_region_data *)NULL;
...
next_rect:
	...
	for (j = num_ri, rit = ri; --j >= 0; rit++) {
		...
		if (reg->data->count_boxes == 1) { /* keep unions happy below */
			FREE_DATA(reg);
			reg->data = (struct clv_region_data *)NULL;
		}
	}
			  ...
			  const struct clv_box *boxes, s32 count)
{
	...
	if (count == 1) {
		clv_region_init_rect(region, boxes[0].p1.x, boxes[0].p1.y,
	...
	memcpy(inner_boxes, boxes, sizeof(struct clv_box) * count);
...
{
	if (!GOOD_BOX(extents)) {
		clv_err("Invalid box, %d,%d %d,%d", extents->p1.x,
...
static void set_extents(struct clv_region *region)
{
	struct clv_box *box, *box_end;
...
void clv_region_translate(struct clv_region *region,
			     s32 x, s32 y)
{
	u32 count_boxes;
	struct clv_box *box;
...
s32 clv_region_copy(struct clv_region *dst, struct clv_region *src)
{
	...
	memmove((char *)REGION_BOX_PTR(dst), (char *)REGION_BOX_PTR(src),
		 dst->data->count_boxes * sizeof(struct clv_box));

	return 0;
}

s32 clv_region_intersect(struct clv_region *new_reg,
			    struct clv_region *reg1,
			    struct clv_region *reg2)
{
	...
	if (REGION_NIL(reg1) || REGION_NIL (reg2)
		...
		return clv_region_copy(new_reg, reg1);
	} else if (!reg1->data && SUBSUMES(&reg1->extents, &reg2->extents)) {
		return clv_region_copy(new_reg, reg2);
	} else if (reg1 == reg2) {
		return clv_region_copy(new_reg, reg1);
	} else {
...
s32 clv_region_intersect_rect(struct clv_region *dst,
				 struct clv_region *src,
				 s32 x, s32 y, u32 w, u32 h)
{
	struct clv_region region;
	...
	return clv_region_intersect(dst, src, &region);
}

s32 clv_region_union(struct clv_region *new_reg,
			struct clv_region *reg1,
			struct clv_region *reg2)
{
...
s32 clv_region_union_rect(struct clv_region *dst,
			     struct clv_region *src,
			     s32 x, s32 y, u32 w, u32 h)
{
	struct clv_region region;
	...
	if (!GOOD_BOX(&region.extents)) {
		if (BAD_BOX(&region.extents))
			clv_err("Invalid rectangle passed");
		return clv_region_copy(dst, src);
	}

	region.data = NULL;

	return clv_region_union(dst, src, &region);
}

s32 clv_region_subtract(struct clv_region *reg_d,
			   struct clv_region *reg_m,
			   struct clv_region *reg_s)
{
	...
	if (REGION_NIL(reg_m) || REGION_NIL(reg_s)
		...
		return clv_region_copy(reg_d, reg_m);
	} else if (reg_m == reg_s) {
		FREE_DATA(reg_d);
